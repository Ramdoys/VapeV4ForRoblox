...
run(function()
	...
	bd = setmetatable({
		...
		__index = function(self, ind)
			rawset(self, ind, string.find(ind, 'Service') and Knit.GetService(ind) or Knit.GetController(ind))
...
run(function()
	...
	local ParticleSize
	local LegitAura
	local Particles, Boxes, AttackDelay, SwingDelay, ClickDelay = {}, {}, os.clock(), os.clock(), os.clock()
	...
	local function getAttackData()
		...
		if LegitAura.Enabled then 
			if ClickDelay < os.clock() then return false end
		end
					...
					if tool and rawget(bd.CombatConstants.DAMAGE, tool.Name) then
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								if not Swing.Enabled and SwingDelay < os.clock() then
									SwingDelay = os.clock() + 0.25
				...
				repeat
					if entitylib.isAlive then
						...
						if LimitItem.Enabled then 
							tool = getTool()
							tool = tool and string.find(tool.Name, 'Block')
						end
